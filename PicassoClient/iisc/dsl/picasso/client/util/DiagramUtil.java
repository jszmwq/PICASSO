...
//   	 Create a flat array for plans
		...
		if(gdp.getQueryPacket().getDistribution().startsWith(PicassoConstants.UNIFORM_DISTRIBUTION))
		{
			...
			else
			{
				...
				String filePath = "/home/liuziang/Desktop/plandiagram/GRID/grid.txt";
				...
				try {
					...
					for (int i=0; i < NROWS; i++) {
						for (int j=0; j < NCOLS; j++) {
							...
							writer.newLine();
							writer.write(lb[0] + "," + lb[1]);
							writer.newLine();
							writer.write(lt[0] + "," + lt[1]);
							writer.newLine();
							writer.write(rt[0] + "," + rt[1]);
							writer.newLine();
							writer.write(rb[0] + "," + rb[1]);
							writer.newLine();
						}
					}
					writer.close();
