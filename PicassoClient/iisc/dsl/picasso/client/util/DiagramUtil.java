...
public class DiagramUtil {
	...
	public static double[][] getSamplesForPlan(int type, MainPanel panel, DiagramPacket gdp) throws IOException {
				...
				try {
					...
					for (int i=0; i < NROWS; i++) {
						for (int j=0; j < NCOLS; j++) {
							...
							grids[i][j] = planid;
							x = sValues[j];
							y = sValues[i + gdp.getResolution(0)];
							lb[0] = x - (float)100/(NCOLS*2);
							lb[1] = y - (float)100/(NROWS*2);
							lt[0] = x - (float)100/(NCOLS*2);
							lt[1] = y + (float)100/(NROWS*2);
							rt[0] = x + (float)100/(NCOLS*2);
							rt[1] = y + (float)100/(NROWS*2);
							rb[0] = x + (float)100/(NCOLS*2);
							rb[1] = y - (float)100/(NROWS*2);
							writer.write("next\n");
							writer.write(String.valueOf(planid));
							writer.newLine();
							writer.write("sep\n");
							writer.write("4\n");
							writer.write(df.format(lb[0]) + " " + df.format(lb[1]));
							writer.newLine();
							writer.write(df.format(lt[0]) + " " + df.format(lt[1]));
							writer.newLine();
							writer.write(df.format(rt[0]) + " " + df.format(rt[1]));
							writer.newLine();
							writer.write(df.format(rb[0]) + " " + df.format(rb[1]));
							writer.newLine();
						}
					}
					writer.close();
					System.out.println(" Record grids " );
				} catch (IOException e) {
					e.printStackTrace();
				}
