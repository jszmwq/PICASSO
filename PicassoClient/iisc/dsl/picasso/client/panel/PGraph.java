...
public class PGraph extends JPanel implements ActionListener, MouseListener, ItemListener {
...
//    boolean				isMultiple;
    ...
    JTextArea			suboperatorLbl;
    JButton 			zoomin, zoomout;
    JComboBox			disptypeBox;
    JScrollPane 		scroller=null;
    int					planId;
    Vector				trees;
    PlanTreeFrame		parent;
    DefaultGraphCell[]  cells;
    
    public PGraph(PlanTreeFrame ptf, int pid, String[] tn, int[] tc, Vector ts, int[][] sortedPlan, boolean im, int msg, String[] infoStr) {
        treeNames = tn;
        treecard=tc;
...
//		Zoom In and Zoom Out
        ...
        JLabel disptypeLbl = new JLabel("Display Type: ", JLabel.RIGHT);
        disptypeBox = new JComboBox();
        disptypeBox.addItem("Both");
        disptypeBox.addItem("Cost");
        disptypeBox.addItem("Cardinality");
        disptypeBox.addItem("None");
        disptypeBox.addItemListener(this);
        if ( msgId == MessageIds.GET_COMPILED_PLAN_TREE ) {
            disptypeLbl.setVisible(true);
            disptypeBox.setVisible(true);
            disptypeBox.setSelectedIndex(parent.displayType);
        } else {
            disptypeLbl.setVisible(false);
            disptypeBox.setVisible(false);
            disptypeBox.setSelectedIndex(3);
        }
        
        // String str = System.getProperty("os.name");
        String str = System.getProperty("os.name");
        JTextField colordot = new JTextField("  ");
        if(str.indexOf("Windows") != -1) // If windows
	        colordot.setBackground(new Color(PicassoConstants.color[clientPlanNumber%PicassoConstants.color.length]));
        else // Anything other than Windows
        {
        	colordot.selectAll();
            colordot.setSelectionColor(new Color(PicassoConstants.color[clientPlanNumber%PicassoConstants.color.length]));
        }
        colordot.setEditable(false);
        
        JLabel plannocostcardLbl = new JLabel("Plan : P" + (clientPlanNumber+1)
        + "     " + infoStr[0], JLabel.LEFT);
        plannocostcardLbl.setFont(new Font(PicassoConstants.LEGEND_FONT, Font.PLAIN, 14));
//		lbl.setForeground(new Color(Constants.color[planColor]));
        
        JTextArea currentpointInfoLbl = new JTextArea("");
        currentpointInfoLbl.setFont(new Font(PicassoConstants.LEGEND_FONT, Font.BOLD, 12));
        currentpointInfoLbl.setBackground(Color.ORANGE);
        currentpointInfoLbl.setEditable(false);
        //MessageUtil.CPrintToConsole("Info :: " + sInfoLbl.getText());
        
        suboperatorLbl = new JTextArea("");
        suboperatorLbl.setFont(new Font(PicassoConstants.LEGEND_FONT, Font.BOLD, 10));
        suboperatorLbl.setForeground(Color.BLACK); //new Color(Constants.color[planColor]));
        suboperatorLbl.setBackground(Color.ORANGE);
        suboperatorLbl.setEditable(false);
        
        JPanel topInfoPanel = new JPanel(new FlowLayout());
        infoPanel.setBackground(Color.ORANGE);
        topInfoPanel.setBackground(Color.ORANGE);
        topInfoPanel.add(disptypeLbl);
        topInfoPanel.add(disptypeBox);
        topInfoPanel.add(colordot);
        topInfoPanel.add(plannocostcardLbl);
        topInfoPanel.add(zoomin);
        topInfoPanel.add(zoomout);
        
        infoPanel.setLayout(new BorderLayout());
        JPanel currentpointInfoPanel = new JPanel();
        currentpointInfoPanel.setBackground(Color.ORANGE);
        
        infoPanel.add(topInfoPanel, BorderLayout.NORTH);
        infoPanel.add(currentpointInfoPanel, BorderLayout.CENTER);
        if ( infoStr[1].length() == 0 ) {
            currentpointInfoPanel.setLayout(new GridLayout(1,1));
        } else {
            currentpointInfoPanel.setLayout(new GridLayout(2,1));
            currentpointInfoPanel.add(currentpointInfoLbl);
        }
        currentpointInfoPanel.add(suboperatorLbl);
        
        this.setLayout(new BorderLayout());
        this.add(infoPanel, BorderLayout.NORTH);
        drawGraph();
        currentpointInfoLbl.setText("   " + infoStr[1] + "\n   " + infoStr[2]);
    }
    ...
    private void drawGraph() {
        ...
        //int dummyWidth = 400 - graph.getPreferredScrollableViewportSize().width;
        //MessageUtil.CPrintToConsole(this.getWidth() + " Adding dummy Panel :: " + dummyWidth);
                /*if ( dummyWidth > 0 ) {
                        JPanel dummyPanel = new JPanel();
                        dummyPanel.setPreferredSize(new Dimension(dummyWidth, this.getHeight()));
                        dummyPanel.setBackground(Color.LIGHT_GRAY);
...
//        MessageUtil.CPrintToConsole(hscale + " Scale :: " + wscale);
    ...
    DefaultGraphCell[] insertTreeData(int planColor, int planNumber, TreeNode root, JGraph graph) {
        Vector nodes = new Vector();
        Vector edges = new Vector();
        //DefaultGraphCell cell =
        insertNodesAndEdges(root, nodes, edges, new Color(PicassoConstants.color[planColor%PicassoConstants.color.length]));
        
        //MessageUtil.CPrintToConsole("Edges " + edges.size() + " Nodes " + nodes.size());
        DefaultGraphCell[] cells = new DefaultGraphCell[nodes.size() + edges.size()];
    ...
    void addHighlightNode(String str, Vector nodes, Vector edges,
        ...
        //GraphConstants.setBounds(cell.getAttributes(), Rectangle2D.(20, 20, 40, 20));
        GraphConstants.setFont(cell.getAttributes(), new Font(PicassoConstants.LEGEND_FONT, Font.BOLD, 10));
    ...
    DefaultGraphCell insertNodesAndEdges(TreeNode tree, Vector nodes, Vector edges, Color c) {
...
//		Get the attribute for similarity..
        ...
        switch (sim) {
            case PicassoConstants.T_IS_SIMILAR :
                ...
                leftEdgeColor = rightEdgeColor = colorBlack;
                break;
        ...
        tree.setDisplayType(disptypeBox.getSelectedIndex());
        //MessageUtil.CPrintToConsole("Tree :: " + tree.getNodeName() + " " + tree.isSimilar());
        DefaultGraphCell cell = createVertex(tree, /*labels,*/ cellColor, borderColor, colorBlack);
        nodes.add(cell);
        
                /*if ( tree.getMatchNumber() != 0 ) {
                        //DefaultGraphCell match = addMatchNode(tree.getMatchNumber());
                        edges.add(createMatchEdge(cell, cell, new Color(PicassoConstants.matchColor[tree.getMatchNumber()])));
                        //nodes.add(match);
                }*/
        ...
        if ( children != null && children.size() != 0 ) {
            ...
            for (int i=0; i < children.size() ; i++) {
                ...
                if ( childNode == null ) {
                    MessageUtil.CPrintErrToConsole("Null Child: Error in Server Tree Creation " + cell.toString());
                    continue;
                }
                //MessageUtil.CPrintToConsole("Child : " + i + " " + childNode.toString());
                DefaultGraphCell child = insertNodesAndEdges(childNode, nodes, edges, c);
                
                //MessageUtil.CPrintToConsole(cell.toString() + " In Insert Edges : " + child.toString());
                if(i<tree.getNodeCard(treeNames, treecard)){
                    if (i==0)
                        edges.add(createEdge(cell, child, leftEdgeColor));
                    else if (i==1)
                        edges.add(createEdge(cell, child, rightEdgeColor));
                    else
                        edges.add(createEdge(cell, child, colorBlack));
                } else
                    edges.add(createDashedEdge(cell, child, colorBlack));
                //i--;
            }
        }
        return(cell);
    ...
    protected DefaultGraphCell addMatchNode(int matchNumber) {
        ...
        GraphConstants.setInset(cell.getAttributes(), 5);
        GraphConstants.setEditable(cell.getAttributes(), false);
        GraphConstants.setBackground(cell.getAttributes(), new Color(PicassoConstants.matchColor[matchNumber]));
        GraphConstants.setForeground(cell.getAttributes(), Color.BLACK);
        GraphConstants.setOpaque(cell.getAttributes(), true);
        GraphConstants.setBounds(cell.getAttributes(), new Rectangle2D.Double(100, 100, 5, 5));
    ...
    protected DefaultEdge createDashedEdge(DefaultGraphCell sourcePort, DefaultGraphCell targetPort, Color c) {
        DefaultEdge edge = new DefaultEdge();
     ...
     */
    protected DefaultGraphCell createVertex(Object userObject, /*Object[] extraLabels,*/ Color c, Color bc, Color fc) {
        ...
        GraphConstants.setOpaque(cell.getAttributes(), true);
        GraphConstants.setBounds(cell.getAttributes(), new Rectangle2D.Double(100, 100, 40, 20));
        ...
        if ( bc == null )
            ...
            GraphConstants.setBorder(cell.getAttributes(),
                    BorderFactory.createEtchedBorder(EtchedBorder.RAISED, bc, bc));
     ...
     * @return the new vertex instance
     */
    protected DefaultEdge createEdge(DefaultGraphCell sourcePort, DefaultGraphCell targetPort, Color c) {
...
//	A Custom Model that does not allow Self-References
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        for (int i=0; i < views.length; i++) {
            if (views[i].getBounds().contains(x/graph.getScale(), y/graph.getScale()) ) {
                ...
                if ( node != null ) {
                    ...
                    suboperatorLbl.setText("   " + node.getNodeName() + ": " + str);
                    MessageUtil.CPrintToConsole("Found Graph Node " + i + ", " + node.toString());
                }
            }
        }
    }
    ...
    public void itemStateChanged(ItemEvent e) {
        if ( e.getSource() == disptypeBox ) {
            curVal = disptypeBox.getSelectedIndex();
            if ( prevVal != curVal ) {
                prevVal = curVal;
                if ( graph != null ) {
